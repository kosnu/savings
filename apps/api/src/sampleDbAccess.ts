...
export async function touch(env?: Env) {
  const dbFile = typeof process !== 'undefined' && process.env ? process.env.DB_FILE_NAME : undefined;
  if (!dbFile && !env?.DB) {
    throw new Error('Environment variable DB_FILE_NAME is required (or provide Env.DB)');
  }
