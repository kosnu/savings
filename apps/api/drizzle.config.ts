...
const allowLocal =
  String(process.env.ALLOW_LOCAL_DB ?? "").toLowerCase() === "true"
...
// biome-ignore lint/style/noNonNullAssertion: allowLocal が true の場合、dbFile は保証される
const dbUrl = databaseUrl ?? dbFile!

// Cloudflare D1 の databaseId（任意）。環境変数で上書き可能です。
const defaultDatabaseId =
  process.env.D1_DATABASE_ID ?? "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
