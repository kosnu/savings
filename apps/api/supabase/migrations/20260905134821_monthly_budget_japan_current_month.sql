-- 認可月はDBのstatement開始時刻を日本時間へ変換して固定する。
-- 日付だけの作成対象月はtimezone変換しない。既存RPC署名・認可・履歴遷移は維持する。
...
begin
  ...
  effective_month_start := date_trunc('month', p_effective_month::timestamp)::date;
  current_month_start := date_trunc('month', statement_timestamp() at time zone 'Asia/Tokyo')::date;
