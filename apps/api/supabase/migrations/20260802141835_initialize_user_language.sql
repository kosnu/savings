drop function if exists public.ensure_authenticated_user(text);

create function public.ensure_authenticated_user(
  p_initial_display_name text,
  p_initial_language text
)
returns void as $$
declare
...
begin
  ...
  if p_initial_language is not null and p_initial_language not in ('en', 'ja') then
    raise exception 'Initial language must be en, ja, or null.';
  end if;
  ...
  insert into public.users (auth_user_id, name, email, language)
  values (
...
revoke all on function public.ensure_authenticated_user(text, text) from public;
revoke all on function public.ensure_authenticated_user(text, text) from anon;
grant execute on function public.ensure_authenticated_user(text, text) to authenticated;
