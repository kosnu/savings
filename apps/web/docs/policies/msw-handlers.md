...
when_to_read:
  ...
  - Suspense や非同期表示のテスト用 mock を追加するか判断するとき
...
fetch 系のテストなど、MSW server/handler を使う `.ts` テストは `.integration.test.ts` にします。

`.integration.test.ts` は jsdom と MSW setup を持つ integration project で実行されます。DOM や MSW が不要な純粋な `.test.ts` は unit project で実行します。
