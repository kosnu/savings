...
- 新しい認証セッションでsource of truthの再取得が必要な場合、前の認証セッションで開始した進行中の取得結果を、新しい認証セッションの再取得完了として扱ってはいけません。新しい認証セッションの準備完了は、そのセッションに帰属する取得の完了またはfallback決定に基づけます。
...
- 回帰テストでは、継続中の認証セッションの更新と、同じユーザーを含む新しい認証セッションを区別し、ユーザー向け表示が適切に維持または再解決されることを確認します。再ログイン時の再取得を検証する場合は、事前の明示的なcache invalidationを前提にしません。また、前の認証セッションの取得が未完了のまま新しい認証セッションへ遷移する経路を含め、旧セッションの結果だけで表示へ復帰しないことを確認します。
