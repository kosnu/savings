...
- クライアントが更新できるユーザープロフィール列は `name` と `language` のみである。
- ユーザープロフィール更新では、認証中のユーザーに対応する1件が更新されたことを確認し、更新対象が見つからない場合を成功として扱わない。
- `name` はユーザー編集可能な表示名であり、監査・権限・本人確認には使わない。
- `language` はユーザー編集可能な言語設定であり、`en`、`ja`、未設定のいずれかとする。監査・権限・本人確認には使わない。
- 初期登録時はユーザーに言語選択を要求せず、`public.users.language` が未設定で `navigator.languages` に対応言語が含まれる場合は、配列順で最初の対応言語を初期値として登録する。
- 登録後は `public.users.language` を言語設定の正本とし、`navigator.languages` は初期登録時の候補に限って使用して、既存値を自動的に上書きしない。
