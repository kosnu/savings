...
- カテゴリ未設定の支払いをUIでどう表現するかは、画面文脈ごとのUIポリシーで決める。
- 支払いに設定するカテゴリは支払いと同じBookに属していなければならない。
- 支払い作成時に所有先Bookが未指定の場合、認証ユーザーのdefault bookを使う。
- 支払いの所有先Bookは更新で変更できない。
- Webの支払い作成、一覧、詳細、更新、削除、「よくある支払い」候補は選択中Bookに限定する。
- Webの支払い作成では選択中Bookを所有先として明示する。
- Webの支払い詳細、更新、削除は支払いIDと選択中Bookの両方で対象を限定する。
- Webの支払いquery cacheは選択中Bookごとに分離する。
