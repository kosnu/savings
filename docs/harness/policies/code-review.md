...
Designはschema v3の`target_state`に、最終product behavior、verification case、正規化したownership scope、最終representationを構造化して所有します。rule coverageはtarget representation pathとownership scope内のbaseline pathの和集合から導出します。Design completion receiptはtarget state、ownership scope、baseline inventory、surface、最終selected rule文書、Build開始前のGit `HEAD`を固定します。

Build完了時は、task-owned範囲の全必須representationが存在し、正本未登録のpath・runtime named export・静的登録test caseが残らず、全verification caseにcase type別の構造化成功証拠があることを先に確認します。default・type-only・`const enum`はruntime named representationを満たさず、import/source-backed・wildcard re-exportは完全inventory不能として拒否します。test caseは承認runnerから非aliasのnamed importだけを使い、module level、または同じrunnerから直接importした`describe`のinline callbackだけを登録経路とします。alias・namespace・default/dynamic import、無効suite、制御構文、未呼出しfunction・到達不能位置を登録済みとして数えません。automated commandの実行fileはrepo allowlist（`pnpm`、Python 3、`node`、`git`、`jq`）に限定します。repo-owned verification runnerは実行前inventoryと各case後のtask-owned state不変を検証して結果を同じfinal-state hashへ固定し、coverage validatorはそのgenerator・state・command・stream境界を含む結果identityを検証します。generator labelとhashはGit・review・CI信頼境界内のcanonical evidenceであり、contributorに対する暗号学的attestationとは扱いません。`file` locatorはfile全体を不可分な1 representationとしてpath単位で照合し、任意の内部entry列挙は保証しません。独立して有無を照合するnamed entryには粒度locatorを使います。baselineにだけ存在するrepresentationは、削除要求を追加せず、target stateとの差として最終成果から除外されていなければ失敗します。ownership scope外の既存ファイルは不純物として扱いませんが、Build差分を作ることも許可しません。VCS metadataはpathの任意segment、Git ignore対象はownership scopeにできず、Build差分を機械観測できないpathを正本へ登録しません。
