...
- レビューコメントが新しいユーザー向け表示、状態、操作を提案しても、現行Task ContextまたはRequirementsで必要性が確定していない限り、実装根拠にしない。AI Driven DevelopmentではTask Context候補としてLearnへ分類し、採用後の次サイクルでのみproduct behaviorへ加える。
- コード、テスト、Storyは、確定済みproduct behaviorを次サイクル以降も維持するための表現として作る。レビュー対応途中の状態や未採用の案には追加せず、Build / Verify完了時には以後の回帰に必要な表現だけを残す。
...
- レビューコメントがpage、auth、network、別ユーザーなど複数境界をまたぐ影響を主張する場合は、確認済みの根拠境界と、提供中の操作列でその影響へ到達できる根拠を分けて確認する。componentなど局所境界で人工的な順序や遅延を再現しただけでは、複数境界をまたぐ影響の到達可能性を確認したことにしない。
- 提供中の操作列での到達可能性が確認できず、局所的な機構だけが成立する低実現性の仮想経路は、プロダクト修正やTask Context変更の対象にしない。対応不要の理由として、確認した境界と不足する到達可能性を返信する。
- テスト階層が低いことだけで対応不要とせず、実際の操作列で到達可能と確認できる指摘は、影響する境界に応じて現在の修正または学びへ分類する。
