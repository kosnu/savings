...
PR本文の「工程別の時間・トークン」へTask ID、各工程の差分と合計、計測の開始・終了時点、
取得できなかった区間と理由を記載し、更新後の本文を読み戻す。PR本文の記録更新と最終応答は
最後の取得値より後に行われるため、計測範囲外として明示する。Goal開始前の準備、Goal toolが
利用できない区間、累積値が取得できない区間は推定やゼロ埋めをせず「取得不可」とする。
この値はCodex Goalが返す使用量であり、実時間や課金額への換算はしない。
