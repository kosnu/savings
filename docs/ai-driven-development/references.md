...
- Ship / Learn Goalでは、PR提案と出荷判断に必要な証跡を明示的に扱う
...
- Ship / Learn Goalでは、検証結果、失敗原因、残リスクを次の改善につなげる
...
- 人間の関与は、各工程の逐次承認ではなく、意図、制約、判断基準、停止条件を与え、例外と出荷判断を監督するために使う
- 小さな変更では4工程を省略してよい
- 4工程目は、commit / PRだけでなく、出荷判断に必要な証跡整理と学習抽出を含める
...
`Ship / Learn Goal` は、PR作成だけでもナレッジ更新だけでもありません。

出荷判断に必要な証拠を揃え、PRD、Design Doc、実装差分、検証結果、残リスクを接続し、次回以降のAI実行単位へ渡すべき学習を抽出する工程です。
