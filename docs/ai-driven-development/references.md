...
`Learn skill` は、Ship完了後にユーザーが手動で実行し、レビューコメント、検証結果、運用知見、ルール・ポリシー変更を次回Requirementsの入力へ整理する補助スキルです。Build / Verifyが正常に完了したらLearnを挟まずShipへ進みます。次回のサイクルを回す場合は、前回の続きとして途中工程から再開せず、`$learn` で入力を整理してからIntent / Requirements Goalを設定します。
