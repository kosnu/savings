...
when_to_read:
  ...
  - PRD、Design Doc、実装、出荷判断の関係を整理するとき
---
...
このフローは、各工程で人間が逐次承認するHuman in the loopではなく、AIがStop条件に当たらない限り前進し、人間が監督、例外処理、出荷判断を担うHuman on the loopを前提にします。
...
- Ship / Learn Goal: 出荷判断可能にし、次のAI実行単位へ学習を渡す
