...
RequirementsとDesignはJSONを機械正本、生成Markdownを人間向け表示とします。cycle-start Issue titleはRequirementsだけが型付きfieldとして所有し、Design以降はRequirementsのhashまたはDesign completion receiptで同じcycleを参照します。machine gateは安定ID、所有者、role、reference、status、hashで完全性と連続性を判定します。Designはproduct behaviorをcanonical Requirementだけが所有するtyped inventoryで定義し、実装予定のmachine review surfaceを構造化して所有します。完了時は完全なDesign Goal・Issue・rule map・最終selected rule文書・成果物・Build基準Git `HEAD`を同じbyte snapshotから検証してreceiptでBuildへ固定します。Buildはその基準点から実差分surfaceを再計算し、Design coverageを超える経路を失敗させます。文章の言い回しや自己申告rule一覧を機械IDの代わりにしません。
