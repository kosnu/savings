...
RequirementsとDesignはJSONを機械正本、生成Markdownを人間向け表示とします。新規サイクルはschema v3を使い、Designの`target_state`をtask-owned範囲の完成状態正本とします。`target_state`は最終product behavior、verification case、ownership scope、machine-addressable representationを保持し、変更・削除の操作一覧は持ちません。Designは現在の実装を要求入力にせず、baseline inventoryとして完成状態との照合とrule選択にだけ使います。完了時は完全なDesign Goal・Issue・rule map・最終selected rule文書・成果物・target state・baseline inventory・Build基準Git `HEAD`をreceiptで固定し、Buildはその凍結済みbaselineを再検証に使います。Buildは既存実装へ差分を足すのではなくtarget stateだけを実体化し、task-owned範囲の未登録pathを不純物として失敗させます。locator metadataからsource構文やtest runner規則は推論しません。v2成果物は履歴表示とbaseline参照に限り、新しいDesign completionまたはBuild入力へ昇格しません。
