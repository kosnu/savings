...
- Rule selection: Issue本文が明示するpath、domain、activity、topic、対象操作、ユーザー向け状態に該当するdirect nodeの和集合と、宣言済み`depends_on` closure。
...
- Issue provenance、全Issue-supported変更面のrule coverage、rule dependency、continuity、canonical path、render同期のgateが成功する。
- 完了直前にIssue snapshotが変わっていない。

## Stop

- Issue、workspace、要求scope、Issue-supported変更面のrule coverage、rule dependencyのいずれかが一意に決まらない。
- Issue snapshotが変化した。
