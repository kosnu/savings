...
- Git boundary: Build Entryから完了まで`HEAD`、current branch、staged treeを変更せず、成果物はworktreeだけへ書く。
...
- repo-owned Build verification runnerがreceipt固定profileのfixed argvだけを専用process groupで実行し、direct runner終了後の残留processを終了・拒否してから、実行前のfinal owned-path inventory、ignore非依存repository mutation manifest、Git `HEAD`のcommitとstaged treeに対する各case後の不変を確認して全`VC-*`のcase type別成功証拠を生成する。current branchの不変はShipだけがbranch変更commandを所有するphase contractとして扱い、checkerの成果物identityには含めない。automated evidenceはprofile ID / hash、typed selector、structured adapterが完全一致を証明したruntime path / full name、exit code 0、stream境界・byte長を固定した出力hash、同一final-state hashを持ち、保存bytesがcanonical JSONと一致する。canonical Build verification / coverage outputはmode `0600`を維持する。repository manifestは`.git` metadataを除くdirectory、regular file、symlinkのtype・permission mode・size・mtime・ctime・device・inodeを含み、ignored fileの作成・変更・削除や作成後削除も拒否する。必須representation pathが存在し、task-owned範囲に未登録pathが残っていない。locator metadataやartifact由来commandからtest runner規則を推論しない。Build Coverageは`HEAD`がreceipt baselineと一致し、staged treeに差分がないことを要求し、`core.fileMode=true`かつ`--no-ext-diff`でbaseline対worktreeを直接検査する。receipt固定untracked baselineから不変の既存untrackedを除外し、新規・変更・削除・tracked化を分類する。Build Entryはreceipt固定baselineとprofile catalogを再検証し、現在のworktreeからbaselineを再計算しない。
...
- Build entry gateを再実行し、Goalへ記録したreceipt SHA-256と一致している。`validate-build`が出力したcanonical Build coverage SHA-256をcompletion evidenceへ記録している。
