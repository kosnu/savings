...
LearnはGoalではなく、Ship完了後にユーザーが必要に応じて手動実行するskillです。Build / Verifyが正常に完了したら、フィードバックの有無を理由にLearnを挟まずShipへ進みます。Ship後の成果物レビュー、レビューコメント、検証結果、運用知見、ルール・ポリシー変更を、Requirementsの材料となるタスクコンテキスト、ルール、ポリシー、監督制約へ整理します。新しいサイクルを回す場合は、前回の続きとして途中工程から再開せず、`$learn` でタスクコンテキストを整理してから、必ずIntent / Requirements Goalから始めます。
...
Build / Verifyが正常に完了したらShipへ進みます。Build / VerifyとShipの間にLearnを挟みません。Ship後の成果物フィードバックは、Build / Verifyを再開して前回実装へ局所修正する入力にせず、ユーザーが手動で`$learn`を実行してRequirementsの材料となるタスクコンテキストへ整理し、新しいサイクルをRequirementsから始めます。
...
Ship完了後に、レビューコメント、検証結果、運用知見、ルール・ポリシー変更をタスクコンテキストまたはルール・ポリシーへ整理します。Learnはユーザーが手動で実行します。
