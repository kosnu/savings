...
- 完了: target stateの全representationが存在し、task-owned範囲に未登録path・runtime named export・test caseが残らない。粒度sourceはtarget_stateの申告種類にかかわらず一度parseしてnamed exportとtest caseを常に両方観測し、実態全体を最終inventoryと照合する。`test_case`はrepository承認runner `vite-plus/test`から非aliasで直接importした`test`/`it`をmodule level、または同じrunnerから非aliasで直接importした`describe`のinline non-generator callbackで静的登録し、各case自身もinline non-generator function callbackを持つ。alias・namespace・default/dynamic import、同名shadowing、callback欠落、generator callback、到達不能登録、suite/caseの`only`/`skip`/`todo`/`fails`を拒否する。parameterized caseは静的に非空なarray-form `each`とinline non-generator function callbackだけを許可する。repo-owned Build verification runnerは実行前にfinal inventoryを検証し、各case後にtask-owned regular fileのpath・Git実行mode・contentとtarget stateからなるfinal-state manifestが不変の場合だけ成功証拠を同じhashへ固定する。実差分がownership scope内でreceiptのrule coverageを満たし、Build entry gate再実行で上流成果物とreceipt固定baselineが不変である。coverage validatorはartifact由来commandを実行しない。generator labelとhashはGit・review・CI信頼境界内のcanonical evidence identityであり、編集権限を持つcontributorに対する暗号学的attestationとは扱わない。
