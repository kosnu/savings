...
Issue本文で期待する状態、期待される挙動、成功条件、完了の定義などを記述する欄では、達成後または操作後に観測できる結果・状態を書きます。実装や作業の手順を実施したこと自体は、その状態へ到達した根拠になりません。状況やユーザー操作は結果が起きる条件として記載できますが、判定対象はその後の期待結果です。

再現手順など、アクションを書く目的の欄では手順を記載し、状態・結果を記述する欄と役割を混同しません。実装手順や詳細な検証手順をIssueへ一律に要求しません。この原則はIssue種別や欄の名称を問わず適用し、Feature Requestの「期待する状態」「成功条件」、Bug Reportの「期待される挙動」「実際の挙動」、Taskの「完了の定義」も対象です。Bug Reportでは前者に期待する結果、後者に発生時に観測した結果・状態を記載します。
