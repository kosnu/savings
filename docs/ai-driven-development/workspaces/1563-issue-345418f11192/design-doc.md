...
- Requirements SHA-256: `8cb0f90cd345b2800015e5af8f3d461f9bd57d509a6d8e4b2e037c43a65f310a`
- Requirements Input Gate / Completeness Gate: artifact検証成功。
- Git `HEAD`の同workspace Design baseline SHA-256: `145793d110ad706c7bdc8af082b7a0e5bb2e52e0107aea5f9cd6a9cd18c018c1`。
...
- Dependency closure: `documentation.policy`、`architecture.overview`、`policy.transaction-boundaries`。Design固有の`additional_rules`はない。
- Conflict decision: write失敗とwrite後の確認失敗を分ける。前者はrollbackとユーザー通知、後者は非rollbackと再確認にし、profile update allowlistは広げない。
...
`LanguageSelect`はwrite失敗とwrite後の確認失敗を別stateとして扱う。write失敗では直前言語へrollbackしたうえで、選択欄の直下に既存Callout形式の保存失敗alertを表示する。次の変更開始時に古いwrite失敗alertを消し、再試行可能なselectorは維持する。確認失敗では既存どおり非rollback・selector無効化・確認専用retryを維持する。

### Feature境界

profile feature外のlanguage providerは、profile rootの`index.ts`公開面からquery keyとfetchを参照する。

architecture baseline: 採用する構成 baselineを、write失敗時のrollback後Calloutと確認失敗の別stateを含むUI責務へ置換する。
...
state\-flow baseline: 状態遷移と優先順位 baselineを、write失敗時のrollback・alert・再試行可能状態を含む遷移へ置換する。
...
FR\-5 design: 未設定、認証世代に帰属する取得失敗、write失敗、write成功後確認失敗を別状態にする。write失敗はrollbackとユーザー向けalertへ変換し、確認失敗はLanguageSelectへ伝播して未確認中の追加writeを止め、確認用refetchだけで収束可能にする。
...
AC\-5 design: 未設定、新しい認証世代に帰属する取得失敗、write失敗、確認失敗を個別のfallback、rollback、保存失敗alert、確認失敗通知、再確認境界へ分解する。
...
requirement\-design baseline: 要求別設計根拠 baselineを、write失敗のrollback後も保存失敗alertを表示する要求所有へ置換する。
...
FR\-3 verification: selector変更のpayload、write失敗時の復元と保存失敗alert、write成功後refetch失敗時の非rollbackをunit・integration testで確認する。
FR\-4 verification: アカウント値ありなしと端末値ありなしの組合せで、採用値とlocalStorage同期結果を表にして検証する。
FR\-5 verification: write失敗時はrollback後も保存失敗alertが表示され、selectorで再試行できることを確認する。write成功後の確認失敗では非rollback、別通知、selector無効化、refetchだけの再確認、成功後の復帰を確認する。
...
AC\-5 verification: 新しい認証世代に帰属する取得、write、write後確認の各失敗でアプリが利用不能にならず、write失敗のrollback後に保存失敗alertが残ること、確認失敗の別表示、再確認失敗時の継続、再確認成功時の収束を確認する。
...
verification baseline: 検証方針 baselineを、write失敗時のrollbackと保存失敗alertを同時に確認する回帰testへ置換する。

## 変更対象

- `apps/web/src/features/preferences/appearanceSettings/LanguageSelect/LanguageSelect.tsx`: write失敗stateを追加し、rollback後に既存Callout形式の保存失敗alertを表示する。次の変更開始時に古いalertを消す。
- `apps/web/src/i18n/resources.ts`: 保存失敗alertの日本語・英語翻訳を追加する。
- `apps/web/src/features/preferences/appearanceSettings/AppearanceSettings/AppearanceSettings.test.tsx`: write失敗時のrollback、alert、再試行可能状態を既存testへ追加する。
- DB、RLS、profile write・確認、認証世代、初回同期の既存契約は変更しない。
- Requirementsと生成requirements.mdはread-onlyとし、Design JSON/Markdownは同期生成する。

implementation\-scope baseline: 変更対象 baselineを、LanguageSelectの保存失敗alert、翻訳resource、既存AppearanceSettings回帰testへ限定して置換する。
...
- write失敗とwrite後確認失敗を単一booleanへ統合すると非rollback・retry契約を壊すため、別stateを維持する。
- 古いwrite失敗alertが次の試行結果と誤認されないよう、次のselector変更開始時に消す。
- DB、RLS、profile write・確認、認証世代、初回同期の既存契約に不整合が見つかった場合はDesign scopeを越えて修正せず停止する。

risks baseline: リスクと確認事項 baselineを、write失敗の例外消費による無通知と確認失敗stateの誤統合を防ぐ観点へ置換する。
