...
- Requirements SHA-256: `3c22e7d31cd1a54722fc6485dbd30c5b9969673da59d2e224db4319ae98d8585`
- Requirements Input Gate / Completeness Gate: artifact検証成功。
- Git `HEAD`の同workspace Design baseline SHA-256: `81fa853170dcfc7784c6ec4119febb4dc49282869dea469c86955f2b1cb30bc9`。
...
- Session boundary: 継続中のtoken更新では認証世代を維持し、サインアウトを挟む新しい認証セッションでは同じユーザーでも世代を更新して、cache freshnessに依存せずprofileを再取得する。
- Conflict decision: profile update allowlistはnameとlanguageだけを許可し、他の列へ広げない。

rule\-selection baseline: Rule Selection baselineを、新しい認証セッションでのfresh取得責務とtoken更新の継続条件へ置換する。
...
`LanguageSyncProvider`はユーザーIDと認証世代をready identityにし、世代ごとにprofileを明示refetchする。cache値だけではreadyにせず、refetchの最新値を適用するか取得失敗・未設定のfallbackを決定してからchildrenを解放する。token refreshでは世代が変わらないため既存表示を維持する。

### Feature境界

profile feature外のlanguage providerは、profile rootの`index.ts`公開面からquery keyとfetchを参照する。

architecture baseline: 採用する構成 baselineを、認証世代の所有境界と世代ごとの明示refetchへ置換する。
...
3. `LanguageSyncProvider`は認証世代が変わるたびにprofileを明示refetchし、refetchが解決するまでその世代を未確認としてchildrenを表示しない。既存cacheがfreshでも完了扱いにしない。
4. account languageがあれば非同期適用完了後にuser IDと認証世代をreadyとして記録する。nullまたは取得失敗なら端末値か既定値をfallbackとして確定後に同じreadyを記録し、推測値はaccountへ保存しない。
...
state\-flow baseline: 状態遷移と優先順位 baselineを、認証世代ごとの明示refetchとready解放条件を含む内容へ置換する。

## 要求別設計根拠

FR\-1 design: 永続化列、migration、生成型、プロフィール契約を同じ所有責務へ結び付ける。
FR\-2 design: 認証ライフサイクル所有者が新しい認証世代を発行し、profileの明示refetchとaccount language適用の完了をuser ID・世代単位のreadyへ記録する。
...
AC\-2 design: ログイン初期化では認証世代ごとのfresh profileとlanguage適用が完了するまで表示を待機し、同一世代のtoken refreshでは表示を継続する。
AC\-3 design: 変更イベント、profile write、source of truth再取得を順序付け、writeと確認の結果を別に保持し、確認再試行ではwriteを繰り返さない。
AC\-4 design: 同じユーザーの再ログインでも認証世代ごとのfresh account取得をready条件にし、fresh cacheよりsource of truthを優先する。
...
AC\-8 design: 初回またはsign\-out後の再ログインでは認証世代を更新してfresh値適用前にchildrenを表示せず、同一世代のtoken refreshでは既に正しいchildrenを隠さない。
AC\-9 design: profile取得失敗をfallback決定としてready stateへ反映し、端末値または既定値でchildrenを表示する。
requirement\-design baseline: 要求別設計根拠 baselineを、FR\-2が所有する認証世代とfresh profile取得のready境界へ置換する。

## 検証方針

FR\-1 verification: migration、生成型、プロフィール取得更新の整合と所有行制約をAPI・DBテストで確認する。
FR\-2 verification: 同一ユーザー再ログインで事前invalidateなしにprofile GETが発生し、fresh値適用前はchildrenが表示されず、同一認証世代のtoken refreshでは表示が継続することを確認する。
...
AC\-2 verification: ログインfixtureで認証世代が初回認証とsign\-out後の再ログインで進み、token refreshでは維持されることを確認する。
AC\-3 verification: 確認再試行でPATCHが増えずGETだけが実行され、成功時に未確認状態が解消して再取得値へ収束することを検証する。
AC\-4 verification: fresh cache期間内の再ログインでserver応答を変更し、事前invalidateなしにGETして最新account値へ収束することをintegration testで確認する。
...
AC\-8 verification: 端末en、cached en、account jaの条件で、token refreshは表示を維持し、sign\-out後の同一ユーザー再ログインはGET完了まで待機することを確認する。
AC\-9 verification: profile GET失敗時は初期待機後にfallback言語でchildrenが表示され、利用可能になることを確認する。
verification baseline: 検証方針 baselineを、事前invalidateなしの同一ユーザー再ログインとtoken refreshを分離する回帰testへ置換する。

## 変更対象

- `apps/web/src/providers/supabase/SupabaseSessionProvider.tsx`: 初回認証、別ユーザー切替、sign-out後の再ログインで進み、同一ユーザーのtoken refreshでは維持される`authenticationGeneration`をsession stateへ追加する。
- `apps/web/src/providers/language/LanguageSyncProvider.tsx`: user IDと認証世代をready identityにし、世代ごとの明示profile refetchが解決してからlanguage適用またはfallbackでchildrenを解放する。
- `apps/web/src/providers/supabase/SupabaseSessionProvider.test.tsx`: 認証世代の更新・維持条件を検証する。
- `apps/web/src/providers/language/LanguageSyncProvider.test.tsx`: 事前invalidateを削除し、fresh cache期間内の同一ユーザー再ログインでGET、待機、最新言語反映を検証する。
- `apps/web/src/test/test-utils.tsx`とsession context fixture: 新しいsession state契約へ同期する。
- DB、API、RLS、言語変更write・確認の既存契約は変更しない。
- RequirementsとDesignのcanonical JSON/MarkdownはDesign完了後にread-onlyとする。

implementation\-scope baseline: 変更対象 baselineを、認証世代を所有するsession provider、language同期、関連fixtureと回帰testへ限定して置換する。

## リスクと確認事項

- Sessionオブジェクトやuser IDだけではtoken refreshと同一ユーザー再ログインを区別できないため、認証ライフサイクル所有者が明示的な世代を提供する。
- 新しい認証世代でcache値を先にready扱いすると古い言語が表示されるため、明示refetchの解決をready条件に含める。
- token refreshごとに世代を進めると不要な待機とGETが発生するため、継続中の同一ユーザーセッションでは世代を維持する。
- 再ログインtestで事前invalidateすると欠陥を隠すため、fresh cacheを残したままserver応答変更とGET発生を確認する。
- DB、API、RLS、言語変更write・確認の既存契約に不整合が見つかった場合はDesign scopeを越えて修正せず停止する。

risks baseline: リスクと確認事項 baselineを、token refreshと同一ユーザー再ログインの誤同一視を防ぐ観点へ置換する。
