...
func main() {
	...
	m, err := migration.CheckScope(ctx, os.Getenv("GITHUB_WORKSPACE"), os.Getenv("PR_BASE_SHA"), os.Getenv("PR_HEAD_SHA"))
	if err == nil {
		err = migration.CheckApproval(migration.GitHubAPI(ctx), os.Getenv("GITHUB_REPOSITORY"), os.Getenv("PR_NUMBER"), os.Getenv("GITHUB_RUN_ID"), os.Getenv("PR_BASE_SHA"), os.Getenv("PR_HEAD_SHA"), *approved)
	}
	if err != nil {
