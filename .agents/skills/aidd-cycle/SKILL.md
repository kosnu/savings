...
3. Give the phase agent a self-contained `message` containing the repository root,
   ...
   read/write boundary, Verification, and Stop conditions. Encode those values
   in a schema-v1 `phase_assignment` draft owned by the parent, then run
   ...
   validated bytes for same-phase continuation unless the parent prepares and
   validates a replacement with the same cycle and Goal identities.
4. The phase agent validates the parent-prepared assignment with the same
   `validate-phase-assignment` command before any phase action and executes only
   the typed active Goal and boundaries in those bytes. It must not invoke
