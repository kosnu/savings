...
- inventory the paths currently present in the ownership scopes and derive
  ...
  baseline inventory in the completion receipt so Build never reconstructs it
  from the changed worktree. The validator adds Requirements-selected rules and
  dependency closure;
...
- require Build to materialize exactly the receipt target state inside the
  ...
  to the current final-state hash over the target state and every owned regular
